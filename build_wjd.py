...
#WORKSPACE = "/Users/admin/workspace/IOS/Wineloan_RN"
WORKSPACEIOS = WORKSPACE + "/" + "ios"
WORKSPACEAndroid = WORKSPACE + "/" + "android"
...
class Build(object):
	...
	def get_target_name(self):
		target_name = os.popen("ls %s | grep xcodeproj | awk -F.xcodeproj '{print $1}'" %WORKSPACEIOS)
	...
	def npm_update(self):
		# Update code and npm version
		print("Update npm and npm version is success")
		os.system("export PATH='$PATH:/usr/local/bin'; export LANG=en_US.UTF-8;LANGUAGE=en_US.UTF-8;LC_ALL=en_US.UTF-8 && cd %s;/usr/local/bin/npm un react-native-yjp-base-common" %WORKSPACE)
		os.system("export PATH='$PATH:/usr/local/bin'; export LANG=en_US.UTF-8;LANGUAGE=en_US.UTF-8;LC_ALL=en_US.UTF-8 && cd %s;/usr/local/bin/npm i -s react-native-yjp-base-common" %WORKSPACE)
		os.system("export PATH='$PATH:/usr/local/bin'; export LANG=en_US.UTF-8;LANGUAGE=en_US.UTF-8;LC_ALL=en_US.UTF-8 && cd %s;/usr/local/bin/npm update" %WORKSPACE)
		#os.system("export PATH='$PATH:/usr/local/bin'; export LANG=en_US.UTF-8;LANGUAGE=en_US.UTF-8;LC_ALL=en_US.UTF-8 && cd %s;/usr/local/bin/react-native link" %WORKSPACE)
		print("Update npm and npm version is success")
		return

	def pod_update(self):
		# Update code and pod version
		os.system("export LANG=en_US.UTF-8;LANGUAGE=en_US.UTF-8;LC_ALL=en_US.UTF-8 && cd %s;/usr/local/bin/pod update" %WORKSPACEIOS)
	...
	def build_app(self):
		...
		work_path = WORKSPACEIOS + '/' + self.target_name + '.xcworkspace'
		plist_file = WORKSPACEIOS + '/' + self.target_name + '.xcodeproj' + '/' + 'project.pbxproj'
		noti_file = WORKSPACEIOS + '/' + self.target_name + '/' + self.target_name + '.entitlements'
		derived_file = WORKSPACEIOS +  '/' +  'build'
#        self.npm_update()
#        self.pod_update()
		self.modify(plist_file,noti_file)
		self.allowkeychain()
		self.cleanPro()
		build_result = os.system("export PATH='$PATH:/usr/local/bin';export PATH='$PATH:/bin';cd %s;/usr/bin/xcodebuild -workspace %s -scheme %s -configuration %s ONLY_ACTIVE_ARCH=NO" %(WORKSPACEIOS,work_path,self.target_name,BUILD_CONFIG))
		if build_result == 0:
			ipa_path = self.ipa_path() + '/' + self.target_name + '-' + BUILD_CONFI
G + '.ipa'
